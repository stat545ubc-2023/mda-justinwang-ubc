...
lets using `head()` first to explore the parking_meters ,`head()` returns the first 6 rows of a dataframe, instead of printing all the data to screen
...
then using `glimpse()` is the dplyr flavour to check the dimensions of the parking-meters dataset
...
I chose the parking_meters because this dataset is so relevant and impact my daily lives. the dataset contains a diverse range of information, including parking rates, time limits, payment methods, location and more. the findings from this analysis can be practical application and can help me make informed decisions about parking costs
...
what is the relationship between parking meter rates such as during 9am - 6pm on weekday and geographic location and is there a big variations in parking rates across different areas?
...
This exercise makes sense to me because it allows me to view the distribution of longitude values, which help me understand the geographical spread of parking meters in the dataset. We can see that parking meters heavily spread between longitude -123.15 and -123.10 in Vancouver.
...
this exercise allows me visualize the relationship between the type of parking meter and whether credits card are accepted. From the plot, we can see that the pay station and twin bay single type of meter are more likely to accept credits card.

### Exercise3: Use a box plot to look at the frequency of different observations within a single variable

I will create a boxplot to examine the distribution of parking rates(r_mf_9a_6p) base on the area
...
This exercise allows us to visually compare the distribution of morning parking rates across different areas. I noticed that fairview and downtown area have higher parking rate and also some of the area don't have sufficient data to generate the box plot such as west point grey.

### Exercise 4: Filter observation in your data according to your own criteria 

Let's filter the dataset to include only parking meters that located in downtown and farview
...
It makes sense because allows me only focus on specific neighborhoods "downtown" and "farview" within the dataset. For example, It allow me to view the differences about the time limit in this area.
